...
const screens={home:$('#homeScreen'),access:$('#accessScreen'),intro:$('#introScreen'),quiz:$('#quizScreen'),loading:$('#loadingScreen'),result:$('#resultScreen'),incomplete:$('#incompleteScreen')};
const show=name=>{Object.values(screens).forEach(x=>x.classList.remove('active'));screens[name].classList.add('active');try{window.scrollTo(0,0)}catch(e){console.warn('Scroll reset unavailable',e)}};
...
let runId=0,answerLocked=false,starting=false,timeoutStreak=0,questionTimedOut=false;
...
let questions=[];
function createEditionQuestions(edition){
 return INTUITION_BANKS[edition].map((q,row)=>({theme:'直觉选牌',title:q.title,desc:q.desc,opts:q.cards.map((label,i)=>['',label,'',v(...q.vectors[i]),row*4+i])}));
}
const readyDecks=new Map();
function loadDeck(edition){
 if(readyDecks.has(edition))return readyDecks.get(edition);
 const promise=new Promise((resolve,reject)=>{
   const img=new Image();let settled=false;
   const done=error=>{if(settled)return;settled=true;clearTimeout(timeout);img.onload=null;img.onerror=null;error?reject(error):resolve()};
   const timeout=setTimeout(()=>done(new Error('牌面加载较慢，请检查网络后重试。本次未扣次数。')),20000);
   img.onload=()=>done();img.onerror=()=>done(new Error('牌面未能加载，请刷新后重试。本次未扣次数。'));img.src=DECK_ASSETS[edition];
 });readyDecks.set(edition,promise);promise.catch(()=>readyDecks.delete(edition));return promise;
}
...
// Legacy entertainment balancing weights; distribution is not recalibrated for the V4.5 question suites.
...
function startQuestion(){
  ...
  $('#phaseText').textContent='凭直觉选一张 · 不必解释';
  try{
    ...
    stage.className=`quiz-stage edition-${mode} spread-${spread} bg-${qIndex%bgCount+1}`;
    ...
    eyebrow.textContent=mode==='china'?'CHINA · INTUITIVE READING':'WORLD · INTUITIVE READING';
    ...
    box.className=`tarot-spread oracle-grid theme-${qIndex+1}`;
    box.innerHTML='';
    q.opts.forEach(([glyph,label,sub,vec],idx)=>{
      const b=document.createElement('button');
      b.type='button';
      b.className='tarot-option oracle-choice';
      b.setAttribute('aria-label',`选择第 ${idx+1} 张牌：${label}`);
      b.style.setProperty('--i',idx);
      b.innerHTML=`<span class="card-surface" aria-hidden="true" style="${deckStyle(mode,qIndex,idx)}"></span><span class="oracle-number">${idx+1}</span>`;
      b.onclick=()=>choose({vec,index:idx},b);
      box.appendChild(b);
    });

    // There are four answer vectors in every current question. Never silently hide options:
    // All four choices must remain visible and available for a deliberate answer.
    ...
    showQuizError('牌阵没有正常展开','本次不会扣除测试次数。请重试；若仍失败，请确认网站文件已完整更新。');
    return false;
  }
}
function showQuizError(heading,message){
    clearInterval(timerId);
    answerLocked=true;
    ...
    if(box){
      ...
      box.innerHTML='<button type="button" class="primary" id="quizRecoveryBtn"><span>返回重试</span><b>↻</b></button>';
      const btn=$('#quizRecoveryBtn');
      if(btn) btn.onclick=()=>{runId++;show('intro')};
    }
    show('quiz');
}
function handleQuestionTimeout(){
  if(!screens.quiz.classList.contains('active')||answerLocked||questionTimedOut)return;
  clearInterval(timerId);questionTimedOut=true;timeoutStreak++;
  answerLocked=true;
  $$('.tarot-option').forEach(x=>x.disabled=true);
  if(timeoutStreak>=2){
    runId++;lastResult=null;$('#timeoutDialog').classList.add('hidden');
    show('incomplete');$('#incompleteTitle').focus();return;
  }
  $('#phaseText').textContent='暂时停一下 · 等待你的选择';
  $('#timeoutDialog').classList.remove('hidden');
  $('#resumeQuestion').focus();
}
$('#resumeQuestion').onclick=()=>{
  if(!screens.quiz.classList.contains('active')||!questionTimedOut)return;
  $('#timeoutDialog').classList.add('hidden');answerLocked=false;
  $$('.tarot-option').forEach(x=>x.disabled=false);
  $('#phaseText').textContent='请继续答题 · 选择后开启下一题';
  $('.tarot-option').focus();
};
$('#timeoutDialog').addEventListener('keydown',e=>{if(e.key==='Tab'){e.preventDefault();$('#resumeQuestion').focus()}});
$('#incompleteHome').onclick=()=>{runId++;show('home')};
$('#incompleteRestart').onclick=()=>beginReading();
...
function choose(ans,el){
  if(!screens.quiz.classList.contains('active')||answerLocked)return;
  answerLocked=true;
  if(!questionTimedOut)timeoutStreak=0;
  ...
  answers.push(ans);
  setTimeout(()=>{if(currentRun!==runId||!screens.quiz.classList.contains('active'))return;qIndex++;if(qIndex<questions.length)startQuestion();else finishTest()},380);
}
function finishTest(){
 clearInterval(timerId);
 if(answers.length!==questions.length){showQuizError('选牌记录不完整','请重新开始，本次未扣除次数。');return}
 try{
  renderResult();
  if(CONFIG.REQUIRE_CODE&&!consumeLocalReport().ok){lastResult=null;show('access');refreshAccessUI('额度已用完，请使用新的授权码。');return}
  show('result');refreshAccessUI();
 }catch(err){lastResult=null;console.error('[Love Oracle] report failed:',err);showQuizError('结果暂时无法展开',err.message||'请重试，本次未扣除次数。')}
}
...
function readAccessStore(){
  try{
    ...
    if(!data||typeof data!=='object'||Array.isArray(data))throw new Error('Invalid access record');
    if(!data.records)data.records={};
    if(typeof data.records!=='object'||Array.isArray(data.records))throw new Error('Invalid access records');
    return data;
  }catch(e){throw new Error('无法读取本机授权记录，请允许网站存储后重试；如记录损坏，请联系商家。')}
}
function writeAccessStore(data){
  try{localStorage.setItem(CONFIG.STORAGE_KEY,JSON.stringify(data));return true}catch(e){throw new Error('无法保存本机授权记录，请允许网站存储后重试。')}
}
...
function getActiveAccess(){
  ...
  if(!rec)return null;
  if(!(CONFIG.LOCAL_CODE_HASHES||[]).includes(hash))return null;
  if(!Number.isInteger(rec.remaining)||rec.remaining<0||rec.remaining>CONFIG.MAX_REPORTS_PER_CODE)throw new Error('本机次数记录异常，请联系商家。');
...
function refreshAccessUI(message=''){
  let a;
  try{a=getActiveAccess()}catch(e){activeCodeHash=null;codeVerified=false;$('#usagePanel').classList.add('hidden');$('#codeMsg').textContent=e.message;return null}
  activeCodeHash=a?.hash||null;
  codeVerified=!!a&&a.remaining>0;
  const panel=$('#usagePanel'),count=$('#remainingCount'),msg=$('#codeMsg');
...
async function verifyAccessCode(value){
  ...
  if(CONFIG.AUTH_API_URL)return {ok:false,error:'backend'};
...
$$('.mode-card').forEach(b=>b.onclick=()=>{
  ...
  mode=b.dataset.mode;
  questions=createEditionQuestions(mode);
  document.querySelector('#app').dataset.edition=mode;
  $('#startBtn').disabled=false;
  $('#startBtn span').textContent=mode==='china'?'继续 · 中国缘分版':'继续 · 世界缘分版';
});
$('#startBtn').onclick=()=>{
  if(!mode)return;
  const access=refreshAccessUI();
  if(!CONFIG.REQUIRE_CODE||(access&&access.remaining>0)){beginReading();return}
  $('#codeMsg').textContent='输入授权码，验证后直接开启牌阵';show('access');
};
$('#accessBackBtn').onclick=()=>show('home');
async function beginReading(){
 if(starting)return;
 if(!INTUITION_BANKS[mode]){show('home');return}
 questions=createEditionQuestions(mode);
 const access=refreshAccessUI();if(CONFIG.REQUIRE_CODE&&(!access||access.remaining<=0)){show('access');return}
 starting=true;timeoutStreak=0;questionTimedOut=false;$('#timeoutDialog').classList.add('hidden');
 show('intro');const currentRun=++runId;clearInterval(timerId);qIndex=0;answers=[];
 const b=$('#beginTest');b.disabled=true;b.classList.add('shuffling');b.querySelector('span').textContent='正在展开牌面…';
 try{
  await Promise.all([loadDeck(mode),new Promise(resolve=>setTimeout(resolve,700))]);
  if(currentRun!==runId)return;if(startQuestion())show('quiz');
 }catch(error){if(currentRun===runId)showQuizError('牌面暂时没有展开',error.message)}
 finally{starting=false;b.disabled=false;b.classList.remove('shuffling');b.querySelector('span').textContent='重新展开牌面'}
}
$('#beginTest').onclick=beginReading;
$('#quitBtn').onclick=()=>{runId++;answerLocked=true;clearInterval(timerId);$('#timeoutDialog').classList.add('hidden');show('home')};
$('#retryBtn').onclick=()=>{
...
$('#verifyCode').onclick=async()=>{
  ...
  let r;
  try{r=await verifyAccessCode(value)}catch(e){$('#codeMsg').textContent=e.message;$('#verifyCode').disabled=false;refreshAccessUI();return}
  ...
  if(r.ok){
    ...
    const access=refreshAccessUI();
    if(access&&access.remaining>0)beginReading();
  }else{
    const text=r.error==='backend'?'此部署包使用本机授权，尚未接入完整后端扣次服务，请联系商家':r.error==='network'?'授权服务暂时不可用，请稍后重试':r.error==='crypto'?'当前浏览器不支持安全校验，请更换现代浏览器':'授权码无效或格式不正确，请检查后重试';
