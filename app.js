...
function startQuestion(){
  ...
  const spread=V4_SPREADS[qIndex%V4_SPREADS.length];
  const stage=$('#quizStage');
  stage.className=`quiz-stage spread-${spread} bg-${qIndex%V4_BG_COUNT+1}`;
  $('#questionEyebrow').textContent=q.eye.replace('INTUITION','DRAW');
  ...
  const box=$('#options'); box.className=`tarot-spread ${spread}`; box.innerHTML='';
